...
string DisplayDayCommand::execute()
{
    for (xml_node<>* train=trains->first_node("Trenuri")->first_node("Tren");train;train=train->next_sibling()){
        ...
        possible.departure=train->first_node("Statie")->first_attribute("Nume")->value();
        possible.arrival=train->last_node("Statie")->first_attribute("Nume")->value();
        for (xml_node<>* station=train->first_node("Statie");station;station=station->next_sibling()){
            ...
            if (name==stationName){
                ...
                if (station==train->first_node("Statie"))
